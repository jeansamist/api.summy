import {
  ...
  type QueueOrder,
  type QueueTask,
} from '#domain/queues/queue_manager_contract'
import { QueueJobFailedException } from '#infrastructure/exceptions/queue_job_failed_exception'
...
interface RetryOptions extends QueueJobOptions {
...
export default class CronManager extends QueueManagerContract {
  ...
  addQueueJob(queueName: string, handler: QueueTask, options: RetryOptions = {}): void {
  ...
  private async executeWithRetry(
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      try {
        ...
        if (isLastAttempt) {
          ...
          throw new QueueJobFailedException(name, maxAttempts, error)
        }
    ...
    throw new QueueJobFailedException(name, maxAttempts, lastError)
  }
